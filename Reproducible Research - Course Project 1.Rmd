...
print(head(activity_data, 25), type="html")
...
var_name <- names(activity_data)
num_col <- ncol(activity_data)
```

There are `r num_col` variables in this dataset. They are `r var_name`.
...
There are `r NA_count` NAs in this dataset.
...
# Sample of mean data
head(Steps_per_time_wk, 25)
# Plot the data using line graphs
ggplot(data = Steps_per_time_wk, aes(x = interval, y = steps)) + geom_line(color = "blue", size = 1) + facet_grid(rows = vars(day_type))
```